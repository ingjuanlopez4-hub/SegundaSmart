...
type Suggestion = { description: string; suggestedPrice: string; source: "ai" | "local" };
...
async function aiSuggestion(input: { name: string; category: string; condition: string; cost: number }): Promise<Suggestion | null> {
  ...
  try {
    ...
    return { description: parsed.description, suggestedPrice: parsed.suggestedPrice.toFixed(2), source: "ai" };
  } catch { return null; }
}
