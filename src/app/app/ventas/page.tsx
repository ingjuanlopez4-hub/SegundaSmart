...
export default async function SalesPage({ searchParams }: { searchParams: Promise<{ producto?: string; pagina?: string | string[]; venta?: string }> }) {
  const user = await requireUser();
  const params = await searchParams;
  const [products, metrics] = await Promise.all([
    db.product.findMany({ where: { businessId: user.businessId, status: "AVAILABLE" }, select: { id: true, name: true, category: true, priceCents: true }, orderBy: { createdAt: "desc" } }),
